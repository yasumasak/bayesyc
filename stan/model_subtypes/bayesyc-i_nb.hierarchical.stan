...
data{
  ...
  array[N1] int y1; // Target cell count with drug1
  array[N2] int y2; // Target cell count with drug2
...
model{
  ...
  sd_a12 ~ gamma(1.2, 0.1);
  sd_a21 ~ gamma(1.2, 0.1);

  log_alpha12 ~ normal(0, sd_a12);
  log_alpha21 ~ normal(0, sd_a21);
...
generated quantities{
  ...
  for (n in 1:Nc) {
    yc_hat[n] = neg_binomial_2_rng(musyc(dc1[n], dc2[n], E0, E1, E2, E3, C1, C2, h1, h2, alpha12, alpha21, gamma12, gamma21, r1r, r2r), 1/s_y);
    log_lik[n] = log_mix(pic[n],
                         neg_binomial_2_lpmf(yc[n] | musyc(dc1[n], dc2[n], E0, E1, E2, E3, C1, C2, h1, h2, alpha12, alpha21, gamma12, gamma21, r1r, r2r), 1/(5 + k)/s_y),
                         neg_binomial_2_lpmf(yc[n] | musyc(dc1[n], dc2[n], E0, E1, E2, E3, C1, C2, h1, h2, alpha12, alpha21, gamma12, gamma21, r1r, r2r), 1/s_y));
  }

  for (n in 1:N1) {
    y1_hat[n] = neg_binomial_2_rng(E1 + (E0 - E1) / (1 + pow(x1[n]/C1, h1)), 1/s_y);
    log_lik[Nc + n] = log_mix(pi1[n],
                              neg_binomial_2_lpmf(y1[n] | E1 + (E0 - E1) / (1 + pow(x1[n]/C1, h1)), 1/(5 + k)/s_y),
                              neg_binomial_2_lpmf(y1[n] | E1 + (E0 - E1) / (1 + pow(x1[n]/C1, h1)), 1/s_y));
  }

  for (n in 1:N2) {
    y2_hat[n] = neg_binomial_2_rng(E2 + (E0 - E2) / (1 + pow(x2[n]/C2, h2)), 1/s_y);
    log_lik[Nc + N1 + n] = log_mix(pi2[n],
                                   neg_binomial_2_lpmf(y2[n] | E2 + (E0 - E2) / (1 + pow(x2[n]/C2, h2)), 1/(5 + k)/s_y),
                                   neg_binomial_2_lpmf(y2[n] | E2 + (E0 - E2) / (1 + pow(x2[n]/C2, h2)), 1/s_y));
  }

  for (n in 1:Nh) {
    yh_hat[n] = neg_binomial_2_rng(E0, 1/s_y);
