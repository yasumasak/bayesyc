...
data{
  ...
  array[N1] real y1; // Target cell viability with drug1
  array[N2] real y2; // Target cell viability with drug2
...
model{
  ...
  for (n in 1:Nc) {
    ...
    target += log_mix(pic[n],
                      normal_lpdf(yc[n] | uc[n], sd_y_out),
                      normal_lpdf(yc[n] | uc[n], sd_y));
  }
  ...
  for (n in 1:N1) {
    ...
    target += log_mix(pi1[n],
                      normal_lpdf(y1[n] | u1[n], sd_y_out),
                      normal_lpdf(y1[n] | u1[n], sd_y));
  }
  ...
  for (n in 1:N2) {
    ...
    target += log_mix(pi2[n],
                      normal_lpdf(y2[n] | u2[n], sd_y_out),
                      normal_lpdf(y2[n] | u2[n], sd_y));
  }
...
generated quantities{
  ...
  array[N1] real y1_hat;
  array[N2] real y2_hat;

  // Pointwise log likelihood
  vector[Nc + N1 + N2] log_lik;

  u_max = E0;

  for (n in 1:Nc_new) {
    uc_pred[n] = musyc(dc1_new[n], dc2_new[n], E0, E1, E2, E3, C1, C2, h1, h2, alpha12, alpha21, gamma12, gamma21, r1r, r2r);
    yc_pred[n] = normal_rng(uc_pred[n], sd_y);
  }

  for (n in 1:Nc) {
    yc_hat[n] = normal_rng(musyc(dc1[n], dc2[n], E0, E1, E2, E3, C1, C2, h1, h2, alpha12, alpha21, gamma12, gamma21, r1r, r2r), sd_y);
    log_lik[n] = log_mix(pic[n],
                         normal_lpdf(yc[n] | musyc(dc1[n], dc2[n], E0, E1, E2, E3, C1, C2, h1, h2, alpha12, alpha21, gamma12, gamma21, r1r, r2r), sd_y_out),
                         normal_lpdf(yc[n] | musyc(dc1[n], dc2[n], E0, E1, E2, E3, C1, C2, h1, h2, alpha12, alpha21, gamma12, gamma21, r1r, r2r), sd_y));
  }

  for (n in 1:N1) {
    y1_hat[n] = normal_rng(E1 + (E0 - E1) / (1 + pow(x1[n]/C1, h1)), sd_y);
    log_lik[Nc + n] = log_mix(pi1[n],
                              normal_lpdf(y1[n] | E1 + (E0 - E1) / (1 + pow(x1[n]/C1, h1)), sd_y_out),
                              normal_lpdf(y1[n] | E1 + (E0 - E1) / (1 + pow(x1[n]/C1, h1)), sd_y));
  }

  for (n in 1:N2) {
    y2_hat[n] = normal_rng(E2 + (E0 - E2) / (1 + pow(x2[n]/C2, h2)), sd_y);
    log_lik[Nc + N1 + n] = log_mix(pi2[n],
                                   normal_lpdf(y2[n] | E2 + (E0 - E2) / (1 + pow(x2[n]/C2, h2)), sd_y_out),
                                   normal_lpdf(y2[n] | E2 + (E0 - E2) / (1 + pow(x2[n]/C2, h2)), sd_y));
  }
